...
streets=['Moiwa st.','Tea st.','Imagination st.','Kojima st.','Mob st.','Main st.','LoveMyself st.','Street st.','HamzaGiveUsA st.']
offices=[]
ssns=[]
appointments=[]
...
num_of_drugs=4
num_of_receipts=20
num_of_appointments=10000
num_of_notifications=37
for i in range (400):                           #append values to offices and ssns lists
    offices.append(i)
    ssns.append(i)
...
def insertStaff():
    for i in range (num_of_staff):
        ...
        hiring_date=datetime.datetime(random.randint(2000,2019),random.randint(1,12),random.randint(1,28),0,0,0)    #randomize all values
        salary=random.randint(30000,60000)
        password=random.choice(passwords)
        first_name = names.get_first_name('any')
        last_name = names.get_last_name()
        birthdate=datetime.datetime(random.randint(1950,1980),random.randint(1,12),random.randint(1,28),0,0,0)
        cursor.execute(f"INSERT INTO Staff(ssn,role_id,office,hiring_date,salary,password,first_name,last_name,birthday) Values({ssn},'{role_id}',{office},TIMESTAMP '{hiring_date}',{salary},'{password}','{first_name}','{last_name}',TIMESTAMP '{birthdate}')")
...
def insertPatient():
    for i in range (num_of_patients):
        ...
        birthdate=datetime.datetime(random.randint(1912,1999),random.randint(1,12),random.randint(1,28),0,0,0)
        gender=random.choice(genders)
        address=random.choice(streets)
        password=random.choice(passwords)
        cursor.execute(f"INSERT INTO Patient(medical_insurance_id,name,birthday,gender,address,password) values({insurance},"
...
def getPatientId():
    ...
    for row in cursor:
        if i==num:
            str=row['medical_insurance_id']         #get medical_insurance_id attribute from the random patient
            return str
        i += 1
def insertMedicalHistoryItem():
    for i in range (num_of_medical_history_items):
        ...
        for row in cursor:
            dateOfB=row['birthday']
        start=datetime.datetime(random.randint(2000,2019),random.randint(1,12),random.randint(1,28),0,0,0)
        symptoms = "Have a nice day! Try to enjoy it..."
        diagnosis = random.choice(illnesses)
        extra = "Extra info"
        if random.randint(0,2)==1:
            end=datetime.datetime(random.randint(start.year,2019),random.randint(1,12),random.randint(1,28),0,0,0)
            cursor.execute(
...
def insertDrug():
    for i in range (num_of_drugs):
        ...
        expire=datetime.datetime(random.randint(2019,2030),random.randint(1,12),random.randint(1,28))
...
def insertReceipt():
    for i in range (num_of_receipts):
        ...
        if validity.month!=12:
            validity=datetime.datetime(validity.year,validity.month+1,validity.day,0,0,0)
        else:
            validity = datetime.datetime(validity.year+1, 1, validity.day)
        patient=getPatientId()
        doctor=getDoctorId()
        used=False
        if random.randint(0,2)==1:
...
def insertNotification():
    ...
    for i in range (num_of_notifications):
        date=datetime.datetime(random.randint(2020,2022),random.randint(1,12),random.randint(1,28),random.randint(8,18),random.randint(0,59))
        patient=getPatientId()
